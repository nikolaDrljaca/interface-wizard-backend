...
async def get_by_id(model_id: str, db=Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Malformed model id.')
    time_format = '%d/%m/%Y-%H:%M:%S' 
    model_metadata = await db.ml_metadata.find_one({'_id': id}, {'_id': 0})
    model_metadata['created'] = datetime.fromtimestamp(model_metadata['created']).strftime(time_format)

    model_metadata['expires'] = datetime.fromtimestamp(model_metadata['expires']).strftime(time_format)
